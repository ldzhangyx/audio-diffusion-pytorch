...
# from diffusion.models.diff_decoder import DiffusionEmbedding, FiLM
...
class MusicTransformer(nn.Module):
    def __init__(self,
        ...
        # self.film = FiLM(4 * attention_ndim, attention_ndim)
        # self.diffusion_embedding = DiffusionEmbedding(2e4, attention_ndim, 4 * attention_ndim)

    def forward(self, x, t=None):
        spec = x.unsqueeze(1)  # batch_size, 1, 128, 992
        h_audio = self.frontend(spec)  # (bs, 62, 256)
        if self.is_vq:
